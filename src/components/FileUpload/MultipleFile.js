...
const MultipleFile = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      for(let i = 0; i < images.length; i++){
        formData.append('multipleFile', images[i])
      }
      axios.post(`http://localhost:5000/api/multiple/image`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
    }).then( 
          response => (response.status === 200 ? setStatus("File uploaded successfully") : setStatus(response.status))
      )

        /***************   example without axios   ***************/      
            //   const response = await fetch('http://localhost:5000/api/multiple/image', {
            //     method: 'POST',
            //     body: formData,
            //   })
            //   if (response) {
